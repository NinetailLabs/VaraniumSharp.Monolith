...
namespace VaraniumSharp.Monolith.Tests.Fixtures
{
    public class TokenGenerator
    {
        ...
        {
            return GenerateToken(DateTime.UtcNow.AddMinutes(-1), DateTime.UtcNow.AddMinutes(10), DateTime.UtcNow, new ClaimsIdentity());
        }

        public string GenerateToken(ClaimsIdentity claimsIdentity)
        {
            return GenerateToken(DateTime.UtcNow.AddMinutes(-1), DateTime.UtcNow.AddMinutes(10), DateTime.UtcNow, claimsIdentity);
        }
        ...
        public string GenerateToken(DateTime? notBefore, DateTime? expires, DateTime? issuedAt, ClaimsIdentity claimsIdentity)
        {
        ...
        [SuppressMessage("ReSharper", "UnusedAutoPropertyAccessor.Local", Justification = "Test class - All properties are required even if not used")]
